...
boxplot(app_data$Joule_calculated, main = "Joules Distribution of entire dataset",  ylab = "Joules", col = "steelblue", border = "black")
dev.off()
```
...
boxplot(Joule_calculated ~ device,
  data = app_data,
  main = "Joules Distribution by app type",
  xlab = "App type",
  ylab = "Joules",
  col = "steelblue",
  border = "black", 
...
?car::qqPlot(differences_high, main="QQ-plot for device type high-end with differences of joules between app type treatments")
dev.off()
```
...
?car::qqPlot(differences_high, main="QQ-plot for device type low-end with differences of joules between app type treatments")
dev.off()
```
...
highRes <- t.test(high_end_web$Joule_calculated, high_end_native$Joule_calculated, paired = T)
highRes$p.value
```
...
low_end_native <- mean_device_app_type_low %>%
  filter(app_type == 'native')
lowRes <- wilcox.test(low_end_web$Joule_calculated, low_end_native$Joule_calculated, paired = T)
lowRes$p.value
```
...
```{r}
# Effect size
cohenResHigh <- cliff.delta(high_end_web$Joule_calculated, high_end_native$Joule_calculated, paired = T, pooled = FALSE)
cohenResLow <- cliff.delta(low_end_web$Joule_calculated, low_end_native$Joule_calculated, paired = T, pooled = FALSE)
