...
```{r}
check_normality(with(mean_device_app_type_high, Joule[app_type == "web"] - Joule[app_type == "native"]))
# Likely to stem from normal distribution
```

Check normality of differences for low end device tier:
```{r}
check_normality(with(mean_device_app_type_low, Joule[app_type == "web"] - Joule[app_type == "native"]))
...
```{r}
par(mfrow=c(1,2))
boxplot(Joule~app_type, data= mean_device_app_type_high, main = "Distribution for a high end device with both app types")
boxplot(Joule~app_type, data= mean_device_app_type_low, main = "Distribution for a low end device with both app types")
```
...
highRes <- t.test(Joule ~ app_type, data = mean_device_app_type_high, paired = T)
lowRes <- t.test(Joule ~ app_type, data = mean_device_app_type_low, paired = T)
highRes$p.value
lowRes$p.value
```
...
```{r}
# Effect size
cohenResHigh <- cohen.d(Joule ~ app_type, data = mean_device_app_type_high, paired = T, pooled = FALSE)
cohenResLow <- cohen.d(Joule ~ app_type, data = mean_device_app_type_low, paired = T, pooled = FALSE)
...
visualize_both <- function(data, title){
  result <- ggplot(data, aes(x = app, y = Joule)) +
  theme_classic() +
...
```{r}
visualize_both(high_end_app_data, "High end data visualized for every app with app_type treatment")
```
...
```{r}
visualize_both(low_end_app_data, "Low end data visualized for every app with app_type treatment")
```
